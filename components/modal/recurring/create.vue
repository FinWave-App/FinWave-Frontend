<template>
  <modal-base :title="$t('modals.newRecurring.title')" :opened="opened" :name="'recurring-create-modal'">
    <div class="w-full flex flex-col gap-2">
      <div class="w-full flex gap-2">
        <div class="form-control w-full">
          <label class="label">
            <span class="label-text">{{ $t('modals.newRecurring.placeholders.recurringAccount') }}</span>
          </label>
        ...
        <div class="form-control w-full">
          <label class="label">
            <span class="label-text">{{ $t('modals.newRecurring.placeholders.recurringCategory') }}</span>
          </label>
      ...
      <div class="form-control w-full">
        <label class="label">
          <span class="label-text">{{ $t('modals.newRecurring.placeholders.recurringAmount') }}</span>
        </label>
      ...
      <div class="form-control w-full">
        <label class="label">
          <span class="label-text">{{ $t('modals.newRecurring.placeholders.recurringRepeatMode') }}</span>
        </label>
      ...
      <div class="form-control w-full">
        <label class="label">
          <span class="label-text">{{ $t('modals.newRecurring.placeholders.recurringNotificationMode') }}</span>
        </label>

        <select class="select select-bordered" v-model.number="notificationMode">
          <option :value="0"> {{ $t('modals.newRecurring.notificationModes.without') }} </option>
          <option :value="1"> {{ $t('modals.newRecurring.notificationModes.silent') }} </option>
          <option :value="2"> {{ $t('modals.newRecurring.notificationModes.normal') }} </option>
        </select>
      </div>

      <div class="form-control w-full">
        <label class="label">
          <span class="label-text">{{ $t('modals.newRecurring.placeholders.recurringNextRepeat') }}</span>
        </label>
      ...
      <div class="form-control w-full">
        <label class="label">
          <span class="label-text">{{ $t('modals.newRecurring.placeholders.recurringDescription') }}</span>
        </label>
...
const create = () => {
  ...
  $recurringTransactionsApi.newRecurring(parentCategory.value, account.value, nextRepeat.value, repeatFunc.value, repeatArg.value, notificationMode.value, amount.value * sign.value, description.value.length > 0 ? description.value : null).then((s) => {
    if (s) {
      $toastsManager.pushToast(t("modals.newRecurring.messages.success"), 2500, "success");
    }else {
      $toastsManager.pushToast(t("modals.newRecurring.messages.error"), 3000, "error");
    }
  });
