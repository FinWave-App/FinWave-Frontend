<template>
  <div class="navbar bg-base-100 shadow max-w-full">
    <div class="navbar-start gap-2">
      <div class="dropdown dropdown-hover z-20">
        <div tabindex="0" role="button" class="btn btn-ghost 2xl:hidden">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h8m-8 6h16" /></svg>
        </div>

        <nav-menu tabindex="0" class="menu menu-compact dropdown-content p-2 px-2 gap-1 shadow bg-base-100 rounded-box w-52">
        </nav-menu>
      </div>
    ...
    <div class="navbar-end mr-4 gap-1">
      <details v-if="aiAvailable" class="hidden lg:block dropdown dropdown-bottom dropdown-end min-w-fit z-20">
        <summary class="btn btn-ghost h-10 px-2">
        ...
        <ai-dialog v-if="!aiModal" :its-dropdown="true" @change-screen="aiModal = true" class="text-sm dropdown-content bg-base-200 bg-opacity-70 backdrop-blur rounded-box z-20 w-96 min-h-96 max-h-screen p-2 shadow-xl"/>
      </details>
